...
TryToEvolve:
	...
	ld hl, Text_CongratulationsYourTohomon
	call PrintText
...
Text_CongratulationsYourTohomon:
	; Congratulations! Your @ @
	text_far _CongratulationsYourTohomonText
	text_end
...
.loop2 ; For each evolution...
	ld a, [hli]
	inc a
	jr z, .no_evolve ; If we jump, this Tohomon does not evolve into wCurPartySpecies.
	dec a
...
.no_evolve
	inc bc
	ld a, b
	cp HIGH(NUM_EXT_TOHOMON)
	jr c, .loop
	ld a, c
	cp LOW(NUM_EXT_TOHOMON)
	jr c, .loop
	and a
	ret
...
INCLUDE "data/tohomon/multi_evos.asm"
